...
| `degree_axes` | int[] | `[]` | – | Axes, by WMX axis number as listed in `joint_axes`, whose user unit is the **degree** (`AxisGearRatioDenominator` 360). Position and velocity for these axes are multiplied by pi/180 before anything is published, so `/joint_states`, `/isaacsim/joint_command` and both Gazebo topics stay in rad and rad/s. Needed by the robot option stack, which the WMX3 SDK requires to run on degree/mm axes — see [reference_robot_option.md](reference_robot_option.md). Listing an axis in both `degree_axes` and `rpm_axes` warns and treats it as degrees. |
